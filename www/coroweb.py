...
def get_required_kw_args(fn):
	...
	#这个字典里的value是一个inspect.Parameter类型的对象，他的值是定义参数时的原样式
	...
	for name, param in params.items():
		if param.kind == inspect.Parameter.KEYWORD_ONLY and param.default == inspect.Pagrmeter.empty:
			args.append(name)
	return tuple(args)
...
def has_request_arg(fn):
	...
	for name, param in params,item():
		...
		if found and (param.kind != inspect.Parameter.VAR_POSITIONAL and param.kind != inspect.Parameter.KEYWORD_ONLY and param.kind != inspect.Paramter.Var_KEYWORD):
			raise ValueError('request parmater must be the last parameter in function: %s%s' % (fn.__name__,str(sig)))
	return found				
...
# RequestHandler的目的是从url函数中分析需要提取的参数,从request中获取必要的参数然后把结果转换为web.Response对象，这样，就完全符合aiohttp框架的要求：
...
class RequestHandler(object):
	...
	async def __call__(self, request):
		...
		if kw is None:
			...
			for k, v in request.mathc_info.items():	#不懂
				if k in kw:
...
def add_routes(app, module_name):
	...
	# dir()不带参数时，返回当前范围内的变量、方法和定义的类型列表；带参数时，返回参数的属性、方法列表。如果参数包含方法__dir__()，该方法
	#将被调用。如果参数不包含__dir__()，该方法将最大限度地收集参数信息。	
	for attr in dir(mod):
		if attr.startwith('_'):
			continue
